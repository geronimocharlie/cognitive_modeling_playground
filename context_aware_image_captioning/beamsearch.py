...
def beam_search(encoder, decoder, image_path, vocab_size, beam_size):
    k = beam_size
...
def beam_search_justify_main(
    encoder, decoder, image_path, class_t, class_d, lambda_, vocab_size, beam_size
):
    ...
    class_embedding_d = decoder.class_embedding(
    ...
    while True:
        ...
        awe_d = gate_d * awe_d
        h_t, c_t = decoder.decode_step(
            torch.cat([embeddings, awe_t, class_embedding_t[:, 0, :]], dim=1),
            (h_t, c_t),
        )  # (s, decoder_dim)
        h_d, c_d = decoder.decode_step(
            torch.cat([embeddings, awe_d, class_embedding_d[:, 0, :]], dim=1),
            (h_d, c_d),
        )  # (s, decoder_dim)
        ...
        if step == 1:
            ...
            # ts)
            # print("scores unrolled")
            # print(scores)
            # print(scores.view(-1).topk(k, 0, True, True))
        else:
            # Unroll and find top scores, and their unrolled indices
            top_k_scores, top_k_words = scores.view(-1).topk(k, 0, True, True)  # (s)
...
def beam_search_discriminative(
    encoder, decoder, image_path_t, image_path_d, lambda_, vocab_size, beam_size=3
):
    ...
    # Flatten encoding
    encoder_out_t = encoder_out_t.view(
        1, -1, encoder_dim
    )  # (1, num_pixels, encoder_dim)
    encoder_out_d = encoder_out_d.view(
        1, -1, encoder_dim
    )  # (1, num_pixels, encoder_dim)
    num_pixels = encoder_out_t.size(1)

    # We'll treat the problem as having a batch size of k
    encoder_out_t = encoder_out_t.expand(
        k, num_pixels, encoder_dim
    )  # (k, num_pixels, encoder_dim)
    encoder_out_d = encoder_out_d.expand(
        k, num_pixels, encoder_dim
    )  # (k, num_pixels, encoder_dim)
    ...
    while True:
        ...
        awe_t = decoder.attention(
            encoder_out_t, h_t
        )  # (s, encoder_dim), (s, num_pixels)
        awe_d = decoder.attention(
            encoder_out_d, h_d
        )  # (s, encoder_dim), (s, num_pixels)
        ...
        h_t, c_t = decoder.decode_step(
            torch.cat([embeddings, awe_t], dim=1), (h_t, c_t)
        )  # (s, decoder_dim)
        h_d, c_d = decoder.decode_step(
            torch.cat([embeddings, awe_d], dim=1), (h_d, c_d)
        )  # (s, decoder_dim)

        # scores = decoder.fc(h_t)- (1-lambda_)*decoder.fc(h_d)  # (s, vocab_size)
        scores = F.log_softmax(decoder.fc(h_t), dim=1) - (1 - lambda_) * F.log_softmax(
            decoder.fc(h_d), dim=1
        )
    ...
    print("number of complete seqs: ", len(complete_seqs))

    if len(complete_seqs_scores) != 0:
        i = complete_seqs_scores.index(max(complete_seqs_scores))
        seq = complete_seqs[i]
    else:
        i = top_k_scores.index(max(top_k_scores))
        seq = seqs[i]
    return seq
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "c":  ## Caption image_path
        assert checkpoint_d is not None, "missing checkpoint"
        ...
        seq = beam_search(encoder, decoder, image_path, vocab_size, 1)
        for i in seq[1:]:
        ...
        seq = beam_search_justify_main(
            encoder, decoder, image_path, int(sys.argv[3]), int(sys.argv[4]), lamb, vocab_size, beam_size
        )
        for i in seq[1:]:
        ...
        seq = beam_search_discriminative(
            encoder, decoder, image_path_t, image_path_d, lamb, vocab_size, beam_size
        )
        for i in seq[1:]:
            print(i, i.numpy())
            print(word_map[i].decode("utf-8"), end=" ")
